...
public class MessageProcessing {
    ...
    public void processMessage(String receivedMessage) {
        ...
        for(String message : splitMessages){
            ...
            if(id.equals("ROBOT2")){
                System.out.println("updating robot 2");
                processRobotLocation2(splitString);
            }else{
                if(id.equals("P")){//POINT codes for debug point, just display it on the screen as a dot
                    processPoint2(splitString);
                }else{
                    if(id.equals("LINE")){
                        System.out.println("updating line 2");

                        processLine2(splitString);
                    }else{
                        if(id.equals("LP")){//log point
                            //add point
                            addPoint2(splitString);
                        }else{
                            if(id.equals("CLEARLOG")){
                                clearLogPoints2();
                            }else{
    ...
    private static double robotX2 = 0;
    private static double robotY2 = 0;
    private static double robotAngle2 = 0;

    public static double getRobotX2() { return robotX2; }
    public static double getRobotY2() { return robotY2; }
    public static double getRobotAngle2() {
        return robotAngle2;
    }
    ...
    public static double getInterpolatedRobotX2(){
        long currTime = System.currentTimeMillis();
        long elapsedSinceLast = currTime - lastClearTime;
        double distanceToCover = robotX2 - lastRobotX2;

        return lastRobotX2 + (distanceToCover * ((double) elapsedSinceLast/lastElapsedTime));
    }
    ...
    public static double getInterpolatedRobotY2(){
        long currTime = System.currentTimeMillis();
        long elapsedSinceLast = currTime - lastClearTime;
        double distanceToCover = robotY2 - lastRobotY2;

        return lastRobotY2 + (distanceToCover * ((double) elapsedSinceLast/50));
    }
    ...
    public static double getInterpolatedRobotAngle2(){
        long currTime = System.currentTimeMillis();
        long elapsedSinceLast = currTime - lastClearTime;
        double distanceToCover = MyMath.AngleWrap(robotAngle2 - lastRobotAngle2);

        return lastRobotAngle2 + (distanceToCover * ((double) elapsedSinceLast/50));
    }
    ...
    private static double lastRobotX2 = 0;
    private static double lastRobotY2 = 0;
    private static double lastRobotAngle2 = 0;
    ...
    private void processRobotLocation2(String[] splitString) {
        if(splitString.length != 4){return;}
        lastRobotX2 = robotX2;
        lastRobotY2 = robotY2;
        lastRobotAngle2 = robotAngle2;


        robotX2 = Double.parseDouble(splitString[1]);
        robotY2 = Double.parseDouble(splitString[2]);
        robotAngle2 = Double.parseDouble(splitString[3]);
    ...
    ArrayList<floatPoint> debugPoints2 = new ArrayList<>();

    //this is used to show paths
    public static ArrayList<floatPoint> pointLog = new ArrayList<>();
    public static ArrayList<floatPoint> pointLog2 = new ArrayList<>();

    ArrayList<Line> debugLines = new ArrayList<>();
    ArrayList<Line> debugLines2 = new ArrayList<>();
     ...
     * @param splitString
     */
    private void processPoint2(String[] splitString) {
        if(splitString.length != 3){return;}
        debugPoints2.add(new floatPoint(Double.parseDouble(splitString[1]),Double.parseDouble(splitString[2])));
    }
     ...
     * @param splitString String[] to be parsed into a point
     */
    private void addPoint2(String[] splitString) {
        if(splitString.length != 3){return;}
        floatPoint toBeAddedMaybe = new floatPoint(Double.parseDouble(splitString[1]),
                Double.parseDouble(splitString[2]));
        //make sure the point doesn't already exist (close enough) in the list
        boolean alreadyExists = false;
        for(floatPoint p : pointLog2){
            if(Math.hypot(p.x-toBeAddedMaybe.x,p.y-toBeAddedMaybe.y) < 1.5){
                alreadyExists = true;
            }
        }
    ...
    private void processLine2(String[] splitString) {
        if(splitString.length != 5){return;}
        debugLines2.add(new Line(Double.parseDouble(splitString[1]),
                Double.parseDouble(splitString[2]),
                Double.parseDouble(splitString[3]),
                Double.parseDouble(splitString[4])));
    }
    ...
    private void clear2() {
        ...
        try {
            Main.drawSemaphore2.acquire();
            Main.displayPoints2.clear();
            Main.displayLines2.clear();

            Main.displayPoints2.addAll(debugPoints);
            Main.displayLines2.addAll(debugLines);
