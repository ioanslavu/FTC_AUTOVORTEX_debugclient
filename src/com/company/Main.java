...
public class Main extends Application {
    ...
    public static Semaphore drawSemaphore2 = new Semaphore(1);
    ...
    public void start(Stage primaryStage) throws Exception {
        //WINDOW STUFF//
        primaryStage.setTitle("AutoVortex 5843 Debug");
        ////////////////
        ...
        UdpUnicastClient udpUnicastClient = new     UdpUnicastClient(5843);
...
//        gc.fillRect(0,0,Screen.widthScreen,Screen.heightScreen);
    ...
    public static ArrayList<floatPoint> displayPoints2 = new ArrayList<>();//all the points to display
    public static ArrayList<Line> displayLines2 = new ArrayList<>();//all the lines to display
    ...
    private void drawDebugPoints2(GraphicsContext gc) {
        for (int i = 0; i < displayPoints2.size(); i++) {
            floatPoint displayLocation = convertToScreen(
                    new floatPoint(displayPoints2.get(i).x, displayPoints2.get(i).y));
            double radius = 5;
            gc.setStroke(new Color(0.0, 1.0, 1.0, 0.6));
        ...
        for (int i = 0; i < MessageProcessing.pointLog2.size(); i++) {
            floatPoint displayLocation = convertToScreen(
                    new floatPoint(MessageProcessing.pointLog2.get(i).x,
                            MessageProcessing.pointLog2.get(i).y));
            double radius = 5;
            gc.setFill(new Color(1.0, 0.0 + (double) i / MessageProcessing.pointLog2.size(), 0, 0.9));
    ...
    private void drawDebugLines2(GraphicsContext gc) {
        for(int i =0; i < displayLines2.size(); i ++){
            floatPoint displayLocation1 = convertToScreen(
                    new floatPoint(displayLines2.get(i).x1, displayLines2.get(i).y1));
            floatPoint displayLocation2 = convertToScreen(
                    new floatPoint(displayLines2.get(i).x2, displayLines2.get(i).y2));
...
//
    ...
    private void drawRobot2(GraphicsContext gc) {
        //robot radius is half the diagonal length
        double robotRadius = Math.sqrt(2) * 18.0 * 2.54 / 2.0;

        double robotX = MessageProcessing.getInterpolatedRobotX2();
        double robotY = MessageProcessing.getInterpolatedRobotY2();
        double robotAngle = MessageProcessing.getInterpolatedRobotAngle2();
